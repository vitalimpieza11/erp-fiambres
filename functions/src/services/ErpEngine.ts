...
export class ErpEngine {
  ...
  static async executeTransaction(params: TransactionParams, db: admin.firestore.Firestore): Promise<string> {
    ...
    const getStockTx = async (productId: string, transaction: admin.firestore.Transaction): Promise<number> => {
    ...
    const evaluateSoftLockTx = async (productId: string, transaction: admin.firestore.Transaction) => {
