...
with open('src/pages/Precios.tsx', 'r', encoding='utf-8') as f:
    content = f.read()

# Replace states
content = content.replace("  const [generalMarginStr, setGeneralMarginStr] = useState('30');", """  const [generalMarginStr, setGeneralMarginStr] = useState('30');
...
old_use_effect = """  // When selected list or presentaciones change, set up the price items from presentaciones only
  useEffect(() => {
...
new_use_effect = """  // Build items function
...
new_create = """      type: 'presentaciones',
      mode: 'auto',
      isActive: true,
      productOverrides: {},
      createdAt: 0,
...
old_handlers = """  const toggleItemExclusion = (id: string) => {
    setPriceItems(priceItems.map(item => item.id === id ? { ...item, excluded: !item.excluded } : item));
  };"""

new_handlers = """  const toggleItemExclusion = (id: string) => {
    setPriceItems(priceItems.map(item => item.id === id ? { ...item, excluded: !item.excluded } : item));
  };
...
old_save = """        if (item.marginStr !== generalMarginStr || item.excluded) {
          overrides[item.id] = {
...
new_save = """        if (item.marginStr !== generalMarginStr || item.excluded || item.mode !== listModeInput || item.mode === 'manual') {
          overrides[item.id] = {
            margin: parseNumber(item.marginStr),
            ...(item.excluded ? { excluded: true } : {}),
            mode: item.mode,
...
old_updatedList = """      const updatedList = {
        name: listNameInput,
...
new_updatedList = """      const updatedList = {
        name: listNameInput,
        target: listTargetInput,
        type: listTypeInput,
        mode: listModeInput,
...
old_getCur = """      if (item.marginStr !== generalMarginStr || item.excluded) {
        overrides[item.id] = {
          margin: parseNumber(item.marginStr),
          ...(item.excluded ? { excluded: true } : {})
        };
      }"""
new_getCur = """      if (item.marginStr !== generalMarginStr || item.excluded || item.mode !== listModeInput || item.mode === 'manual') {
        overrides[item.id] = {
          margin: parseNumber(item.marginStr),
          ...(item.excluded ? { excluded: true } : {}),
          mode: item.mode,
...
old_getListProducts = """  const getListProducts = (margin: number, overrides?: any) => {
...
new_getListProducts = """  const getListProducts = (margin: number, overrides?: any, listType?: 'presentaciones' | 'mercaderias', listMode?: 'auto' | 'manual') => {
...
# For the cards in view mode list
content = content.replace(
...
# Now updating the UI for the form selectors:
# We find: `<Card style={{ marginBottom: '24px' }}>`
# `  <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr', gap: '16px', alignItems: 'end' }}>`
# We'll replace it to use gridTemplateColumns: '1.5fr 1fr 1fr 1fr 1fr 1fr' or break into multiple rows
old_form = """        <Card style={{ marginBottom: '24px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr', gap: '16px', alignItems: 'end' }}>"""
new_form = """        <Card style={{ marginBottom: '24px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px', alignItems: 'end' }}>"""
content = content.replace(old_form, new_form)

# Add Type and Mode selectors before Estado
old_estado = """            <div>
...
# Table Head
old_thead = """                <th style={{ padding: '12px 20px', textAlign: 'left', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Producto</th>
...
new_thead = """                <th style={{ padding: '12px 20px', textAlign: 'left', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>{listTypeInput === 'presentaciones' ? 'Producto' : 'Mercadería'}</th>
...
with open('src/pages/Precios.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
