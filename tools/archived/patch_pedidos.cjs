...
let content = fs.readFileSync('src/pages/Pedidos.tsx', 'utf-8');
...
// 2. State
const state_injection = `  const [status, setStatus] = useState<any>('PENDIENTE');
...
// 6. intention card
const intention_injection = `                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '12px' }}>
...
// 7. Render Accordion
const accordion_code = `          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', padding: '16px' }}>
...
content = content.replace(/          <Table[\s\S]*?\/>\n        \)}/, accordion_code + "\n        )}");

fs.writeFileSync('src/pages/Pedidos.tsx', content, 'utf-8');
