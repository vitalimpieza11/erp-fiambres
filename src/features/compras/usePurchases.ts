...
import type { Purchase, PurchaseItem, Supplier, Product, StockMovement, CajaMovement, SupplierMovement } from '../../types/domain';
...
export function usePurchases() {
  ...
  const addPurchase = async (purchaseData: Omit<Purchase, 'id' | 'isDeleted'>) => {
  ...
  const annulPurchase = async (purchaseId: string, reason: string) => {
    const original = purchases.find(p => p.id === purchaseId);
