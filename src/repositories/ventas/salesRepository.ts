...
export const salesRepository = {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      if (usePackages) {
        ...
        for (const item of itemsToSell) {
          ...
          const targetWeight = item.pesoReal || item.cantidad;
          const selectedForThisItem = [];
          for (const pkg of prodPkgs) {
            if (weightAccumulated >= targetWeight) break;
            selectedForThisItem.push(pkg);
            weightAccumulated += pkg.weight;
          }
          if (weightAccumulated < targetWeight) {
            throw new Error(`No hay paquetes suficientes para cubrir ${targetWeight} kg del producto ${item.productId}.`);
          }
          
          packageDocsToUpdate.push(...selectedForThisItem);
          
          // Guardar IDs en el item si se requiere mapear después
          (item as any).packageIds = selectedForThisItem.map(p => p.id);
        }
      ...
      const mappedItems = itemsToSell.map(i => {
        ...
        return {
          ...
          packageIds: (i as any).packageIds
        };
      });
  ...
  async cobrarSale(sale: Sale, method: 'EFECTIVO_TRANSFERENCIA' | 'CUENTA_CORRIENTE', accountId?: string): Promise<void> {
    if (method === 'EFECTIVO_TRANSFERENCIA' && !accountId) {
      throw new Error("Debe seleccionar una cuenta de caja/banco para registrar el cobro.");
    }
    if (sale.status === 'ANULADO') throw new Error("No se puede cobrar una venta anulada.");
    if (sale.status === 'COBRADO') throw new Error("La venta ya se encuentra cobrada.");
